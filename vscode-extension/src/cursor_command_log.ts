 ...
 * 
 * Note: This is a copy of the shared logger from src/core/cursor_command_log.ts
 * VS Code extensions can't import from the main project, so we duplicate it here.
...
export class CursorCommandLogger {
    ...
    constructor(logPath?: string, enabled: boolean = true) {
        ...
        this.logPath = logPath || path.join(os.homedir(), '.assistant-data', 'cursor_command_log.jsonl');
    ...
    logCommand(
        ...
        try {
            ...
            const duration = metadata?.durationMs || (startTime ? Date.now() - startTime : undefined);
            if (startTime) {
