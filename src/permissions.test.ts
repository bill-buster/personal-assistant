...
if (
    !noConfirmJson ||
    !noConfirmJson.error ||
    !noConfirmJson.error.message.includes('requires confirmation')
) {
    failures += 1;
...
if (
    !noConfirmJson ||
    !noConfirmJson.error ||
    !noConfirmJson.error.message.includes(permissionsFile)
) {
    failures += 1;
    process.stderr.write(
...
if (
    ...
    !disallowedJson.error.message.includes("Tool 'write_file' was blocked")
) {
    failures += 1;
    process.stderr.write(
...
if (
    !disallowedJson ||
    !disallowedJson.error ||
    !disallowedJson.error.message.includes('To unblock, add this path')
) {
    failures += 1;
    process.stderr.write(
...
if (
    ...
    !t3Json.error ||
    !t3Json.error.code ||
    !t3Json.error.code.includes('DENIED_PATH_ALLOWLIST')
) {
    failures += 1;
    process.stderr.write(
...
if (
    ...
    !t4Json.error ||
    !t4Json.error.code ||
    !t4Json.error.code.includes('DENIED_TOOL_BLOCKLIST')
) {
    failures += 1;
    process.stderr.write(
