...
export function readJsonlSafely<T>(options: ReadJsonlOptions<T>): T[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      console.warn(`[JSONL Warning] Skipped corrupt line ${i + 1} in ${filePath}: ${err.message}`);
      if (quarantine) {
  ...
  if (corruptLines.length > 0 && quarantine) {
    ...
    try {
      ...
      console.warn(`[JSONL Recovery] Quarantined ${corruptLines.length} corrupt line(s) to ${corruptPath}`);
    } catch (writeErr: any) {
      console.error(`[JSONL Error] Failed to write to quarantine file ${corruptPath}: ${writeErr.message}`);
    }
  }
