...
import { makeError, makePermissionError, DENIED_TOOL_BLOCKLIST, DENIED_AGENT_TOOLSET, ErrorCode } from './tool_contract';
...
export class Executor {
  ...
  constructor(config: ExecutorConfig) {
    ...
    this.permissionsPath = config.permissionsPath || path.join(this.baseDir, 'permissions.json');
    ...
    for (const p of this.permissions.allow_paths) {
      ...
      const normalized = isDir && resolved.endsWith(path.sep) ? resolved.slice(0, -1) : resolved;
  ...
  private safeResolve(relPath: any): string | null {
    ...
    try {
      ...
      if (canonical !== this.baseDir && !canonical.startsWith(this.baseDir + path.sep)) return null;
      return canonical;
    } catch {
      ...
      try {
        ...
        if (!canonicalParent.startsWith(this.baseDir + path.sep) && canonicalParent !== this.baseDir) return null;
      } catch {
    ...
    for (const entry of this.allowedPaths) {
      if (entry.isDir) {
        if (isCaseInsensitive) {
          ...
          if (targetLower === entryLower || targetLower.startsWith(entryLower + path.sep)) return true;
        } else {
          // Allow if exact match or inside directory (ensure separator check)
          if (targetPath === entry.path || targetPath.startsWith(entry.path + path.sep)) return true;
        }
      } else {
        if (isCaseInsensitive) {
  ...
  private pathResolve(requestedPath: string): string {
    ...
    if (resolved === null) {
      throw makeError(ErrorCode.DENIED_PATH_ALLOWLIST, `Path '${requestedPath}' is invalid or outside baseDir`);
    }
    return resolved;
  }
  ...
  private pathAssertAllowed(targetPath: string, op: PathOp): void {
    if (!this.isAllowedPath(targetPath)) {
      throw makeError(ErrorCode.DENIED_PATH_ALLOWLIST, `Path '${targetPath}' is not allowed for ${op} operation`);
    }
  }
  ...
  private commandRunAllowed(cmd: string, args: string[] = [], _opts?: { confirm?: boolean }): { ok: boolean; result?: string; error?: string; errorCode?: string } {
  ...
  private sortByScoreAndRecency(entries: MemoryEntry[], needle: string, terms?: string[]): MemoryEntry[] {
    const normalizedTerms = terms && terms.length > 0 ? terms : needle.split(/\s+/).filter(Boolean);
  ...
  private runAllowedCommand(commandText: string): { ok: boolean; result?: string; error?: string; errorCode?: string } {
    ...
    if (!allowedCommands.includes(cmd)) {
      const err = makeError(ErrorCode.DENIED_COMMAND_ALLOWLIST, `Command '${cmd}' is not allowed. Listed in permissions.json: ${allowedCommands.join(', ')}`);
    ...
    if (cmd === 'ls') {
      ...
      for (const arg of args) {
        if (arg.startsWith('-')) {
          ...
          if (!safePath) return { ok: false, error: `Invalid path for ls: ${arg}`, errorCode: ErrorCode.INVALID_ARGUMENT };
          if (!this.isAllowedPath(safePath)) {
            const err = makePermissionError('run_cmd', safePath, this.permissionsPath, ErrorCode.DENIED_PATH_ALLOWLIST);
            return { ok: false, error: err.message, errorCode: ErrorCode.DENIED_PATH_ALLOWLIST };
          }
      ...
      if (result.status !== 0) {
        const message = result.stderr.trim() || (result.signal ? `ls terminated by signal ${result.signal}` : 'ls failed');
    ...
    if (cmd === 'pwd') {
      ...
      if (result.status !== 0) {
        const message = result.stderr.trim() || (result.signal ? `pwd terminated by signal ${result.signal}` : 'pwd failed');
        return { ok: false, error: message };
      }
      return { ok: true, result: result.stdout.trim() };
    }
    if (cmd === 'cat') {
      if (args.length !== 1) return { ok: false, error: 'cat requires exactly one path.', errorCode: ErrorCode.MISSING_ARGUMENT };
      if (args[0].startsWith('-')) return { ok: false, error: 'cat flags are not allowed.', errorCode: ErrorCode.INVALID_ARGUMENT };
      const safePath = this.safeResolve(args[0] || '');
      if (!safePath) return { ok: false, error: 'Invalid path for cat.', errorCode: ErrorCode.INVALID_ARGUMENT };
      if (!this.isAllowedPath(safePath)) {
        const err = makePermissionError('run_cmd', safePath, this.permissionsPath, ErrorCode.DENIED_PATH_ALLOWLIST);
        return { ok: false, error: err.message, errorCode: ErrorCode.DENIED_PATH_ALLOWLIST };
      }
      ...
      if (result.status !== 0) {
        const message = result.stderr.trim() || (result.signal ? `cat terminated by signal ${result.signal}` : 'cat failed');
        return { ok: false, error: message };
      }
    ...
    if (cmd === 'du') {
      ...
      for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '-h' || arg === '-s') {
          ...
          if (!next || !/^[0-5]$/.test(next)) {
            return { ok: false, error: 'du -d requires depth 0-5.', errorCode: ErrorCode.INVALID_ARGUMENT };
          }
          ...
          if (!next || !/^-?[0-9]+[kMGT]?$/.test(next)) {
            return { ok: false, error: 'du -t requires valid threshold.', errorCode: ErrorCode.INVALID_ARGUMENT };
          }
          safeArgs.push('-t', next);
          i++;
        } else if (arg.startsWith('-')) {
          return { ok: false, error: `du flag '${arg}' is not allowed. Allowed: -h, -s, -d N (N=0-5), -t SIZE`, errorCode: ErrorCode.INVALID_ARGUMENT };
        } else {
          // Path argument
          const safePath = this.safeResolve(arg);
          if (!safePath) return { ok: false, error: `Invalid path for du: ${arg}`, errorCode: ErrorCode.INVALID_ARGUMENT };
          if (!this.isAllowedPath(safePath)) {
            const err = makePermissionError('run_cmd', safePath, this.permissionsPath, ErrorCode.DENIED_PATH_ALLOWLIST);
            return { ok: false, error: err.message, errorCode: ErrorCode.DENIED_PATH_ALLOWLIST };
          }
      ...
      // Path is required
      if (!pathArg) {
        return { ok: false, error: 'du requires a path argument.', errorCode: ErrorCode.MISSING_ARGUMENT };
      }
      ...
      if (result.status !== 0) {
        const message = result.stderr.trim() || (result.signal ? `du terminated by signal ${result.signal}` : 'du failed');
        return { ok: false, error: message };
      }
      return { ok: true, result: result.stdout.trim() };
    }
    return { ok: false, error: makePermissionError('run_cmd', undefined, this.permissionsPath).message };
  }
  ...
  public async execute(toolName: string, args: any): Promise<ToolResult> {
      ...
      if (this.agent.name === 'System') {
        ...
        if (!this.agent.tools.includes(toolName)) {
          return {
            ok: false,
            result: null,
            error: makeError(DENIED_AGENT_TOOLSET, `Permission denied: agent '${this.agent.name}' cannot use tool '${toolName}'`),
            _debug: makeDebug({ path: 'tool_json', start: nowMs(), model: null, memory_read: false, memory_write: false }),
          };
        }
      }
    }
    ...
    if (this.permissions.deny_tools.includes(toolName)) {
      return {
        ok: false,
        result: null,
        error: makeError(DENIED_TOOL_BLOCKLIST, `Tool '${toolName}' is explicitly denied in permissions configuration.`),
        _debug: makeDebug({ path: 'tool_json', start: nowMs(), model: null, memory_read: false, memory_write: false }),
      };
    }
    ...
    if (!this.agent) {
      ...
      if (!SAFE_TOOLS.includes(toolName)) {
        return {
          ok: false,
          result: null,
          error: makeError(ErrorCode.DENIED_AGENT_TOOLSET, `Permission denied: tool '${toolName}' requires agent context`),
          _debug: makeDebug({ path: 'tool_json', start: nowMs(), model: null, memory_read: false, memory_write: false }),
        };
      }
    }
    ...
    if (schema) {
      ...
      if (!parseResult.success) {
        return {
          ok: false,
          result: null,
          error: makeError(ErrorCode.VALIDATION_ERROR, `Invalid arguments for ${toolName}: ${parseResult.error.message}`),
          _debug: makeDebug({ path: 'tool_json', start: nowMs(), model: null, memory_read: false, memory_write: false }),
        };
      }
    ...
    if (handler) {
      ...
      if (!result || typeof result !== 'object' || typeof result.ok !== 'boolean') {
        return {
          ok: false,
          result: null,
          error: makeError(ErrorCode.EXEC_ERROR, `Internal error: tool '${toolName}' returned no result`),
          _debug: makeDebug({ path: 'tool_json', start: nowMs(), model: null, memory_read: false, memory_write: false }),
        };
      }
    ...
    return { ok: false, result: null, error: makeError(ErrorCode.UNKNOWN_TOOL, `Unknown tool '${toolName}'. Try: ${availableTools}. Use /tools in REPL for full list.`), _debug: null };
  }
