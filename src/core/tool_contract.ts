...
): { tool_name: string; args: Record<string, unknown>; _debug: DebugInfo | null } {
    return {
        tool_name: toolName,
        args: args || {},
        _debug: debug || null,
...
export function validateToolCall(input: unknown): {
    ok: boolean;
    value?: { tool_name: string; args: Record<string, unknown>; _debug: DebugInfo | null };
    error?: ToolError;
} {
    if (!input || typeof input !== 'object' || Array.isArray(input)) {
        return {
            ok: false,
            error: makeError(ErrorCode.VALIDATION_ERROR, 'tool_call must be an object.'),
        };
    }
    ...
    if (!toolName || typeof toolName !== 'string') {
        return {
            ok: false,
            error: makeError(ErrorCode.VALIDATION_ERROR, 'tool_name must be a string.'),
        };
    }
    ...
    if (!spec) {
        return {
            ok: false,
            error: makeError(ErrorCode.UNKNOWN_TOOL, `Unknown tool_name: ${toolName}.`),
        };
    }
    ...
    if (!args || typeof args !== 'object' || Array.isArray(args)) {
        return {
            ok: false,
            error: makeError(ErrorCode.VALIDATION_ERROR, 'args must be an object.'),
        };
    }
            ...
            if (!valid) {
                return {
                    ...
                    error: makeError(
                        ...
                        `Invalid type for arg: ${key}. Expected ${expectedType}.`
                    ),
                };
            }
        }
