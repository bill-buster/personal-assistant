...
// Detect if we're in dist and adjust path accordingly
...
const baseArgs = ['-r', tsNodeRegister, executorPath, '--agent', 'system'];
const routerArgs = ['-r', tsNodeRegister, routerPath];
...
function runRouter(args: string[]) {
  return spawnSync(process.execPath, [...routerArgs, ...args], { cwd: spikeDir, encoding: 'utf8', env: testEnv });
}
...
function runExecutorScript(script: string, args: string[], payload: any) {
  ...
  const result = spawnSync(process.execPath, ['-r', tsNodeRegister, '-e', script, ...fullArgs], {
    cwd: spikeDir,
    input: JSON.stringify(payload),
    encoding: 'utf8',
...
try {
  ...
  if (!writeJson || writeJson.ok !== true || writeJson.tool_name !== 'write_file') {
    failures += 1;
    logLine('FAIL\ncase: write_file\nexpected: ok true\ngot: ' + JSON.stringify(writeJson) + '\nraw: ' + writeResult.stdout + '\n\n', process.stderr);
  }
  if (writeJson) {
    checkDebug(writeJson._debug, { memory_read: false, memory_write: false, path: 'tool_json' });
  }
  ...
  if (!catDoubleQuoteJson || catDoubleQuoteJson.ok !== true || catDoubleQuoteJson.result?.trim() !== quotedFileContent) {
    failures += 1;
  ...
  if (!catSingleQuoteJson || catSingleQuoteJson.ok !== true || catSingleQuoteJson.result?.trim() !== quotedFileContent) {
    failures += 1;
  ...
  if (!unterminatedDoubleQuoteJson || !unterminatedDoubleQuoteJson.error || unterminatedDoubleQuoteJson.error.code !== 'VALIDATION_ERROR') {
    failures += 1;
  ...
  if (!unterminatedSingleQuoteJson || !unterminatedSingleQuoteJson.error || unterminatedSingleQuoteJson.error.code !== 'VALIDATION_ERROR') {
    failures += 1;
    logLine('FAIL\ncase: run_cmd unterminated single quote\nexpected: VALIDATION_ERROR\n\n', process.stderr);
  }
  ...
  if (!duDepthJson || duDepthJson.ok !== true) {
    failures += 1;
    logLine('FAIL\ncase: run_cmd du depth\nexpected: ok true\ngot: ' + JSON.stringify(duDepthJson) + '\nraw: ' + duDepthResult.stdout + '\n\n', process.stderr);
  }
  ...
  if (!combinedFlagsJson || combinedFlagsJson.ok !== true) {
    failures += 1;
    logLine('FAIL\ncase: run_cmd combined flags\nexpected: ok true\ngot: ' + JSON.stringify(combinedFlagsJson) + '\n\n', process.stderr);
  }
  ...
  if (!disallowedJson || !disallowedJson.error || disallowedJson.error.code !== 'DENIED_COMMAND_ALLOWLIST') {
    failures += 1;
  ...
  if (!traversalJson || !traversalJson.error || traversalJson.error.code !== 'DENIED_PATH_ALLOWLIST') {
    failures += 1;
    logLine('FAIL\ncase: path traversal\nexpected: VALIDATION_ERROR\n\n', process.stderr);
  }
  ...
  if (!caseInsensitiveJson || !caseInsensitiveJson.error || caseInsensitiveJson.error.code !== 'DENIED_PATH_ALLOWLIST') {
    failures += 1;
    logLine('FAIL\ncase: case-insensitive blocklist .GIT/config\nexpected: VALIDATION_ERROR\n\n', process.stderr);
  }
  ...
  if (rememberJson) {
    checkDebug(rememberJson._debug, { memory_read: true, memory_write: true, path: 'tool_json' });
  }
  ...
  if (recallJson) {
    checkDebug(recallJson._debug, { memory_read: true, memory_write: false, path: 'tool_json' });
  }
  ...
  const recallRankText = recallRankEntries && recallRankEntries[0] ? recallRankEntries[0].text : null;
  ...
  const limitedRememberResultTwo = runExecutorWithArgs(memoryLimitArgs, limitedRememberPayloadTwo);
  ...
  if (!taskAddJson || taskAddJson.ok !== true || !taskAddJson.result || !taskAddJson.result.task) {
    failures += 1;
    logLine('FAIL\ncase: task_add\nexpected: ok true\n\n', process.stderr);
  }
  ...
  const taskListEntries = taskListJson && taskListJson.result ? taskListJson.result.entries : null;
  ...
  if (!taskDoneJson || taskDoneJson.ok !== true || !taskDoneJson.result || !taskDoneJson.result.task) {
    failures += 1;
    logLine('FAIL\ncase: task_done\nexpected: ok true\n\n', process.stderr);
  }
  ...
  if (!memorySearchJson || memorySearchJson.ok !== true || !Array.isArray(memorySearchEntries)) {
    failures += 1;
    logLine('FAIL\ncase: memory_search\nexpected: entries array\n\n', process.stderr);
  } else if (memorySearchEntries.length === 0 || !memorySearchEntries[0].text.includes('roadmap')) {
    failures += 1;
  ...
  fs.writeFileSync(
    permissionsPath,
    JSON.stringify({ allow_paths: [`./${allowedFile}`], allow_commands: ['du'], require_confirmation_for: ['write_file'] }),
    'utf8'
  );
  ...
  if (!readBlockedJson || !readBlockedJson.error || readBlockedJson.error.code !== 'DENIED_PATH_ALLOWLIST') {
    failures += 1;
    logLine('FAIL\ncase: permissions read blocked\nexpected: VALIDATION_ERROR\n\n', process.stderr);
  }
  ...
  if (!readGitConfigJson || !readGitConfigJson.error || readGitConfigJson.error.code !== 'DENIED_PATH_ALLOWLIST') {
    failures += 1;
    logLine('FAIL\ncase: case-insensitive blocklist (.GIT/config)\nexpected: VALIDATION_ERROR\n\n', process.stderr);
  }
  ...
  if (!writeBlockedJson || !writeBlockedJson.error || writeBlockedJson.error.code !== 'CONFIRMATION_REQUIRED') {
    failures += 1;
    logLine('FAIL\ncase: permissions write confirm\nexpected: CONFIRMATION_REQUIRED\ngot: ' + JSON.stringify(writeBlockedJson) + '\n\n', process.stderr);
  }
  ...
  if (!duBlockedJson || duBlockedJson.ok !== false) {
    failures += 1;
    logLine('FAIL\ncase: permissions du no path\nexpected: ok false\ngot: ' + JSON.stringify(duBlockedJson) + '\nraw: ' + duBlockedResult.stdout + '\n\n', process.stderr);
  }
  ...
  if (!listJson || listJson.ok !== true || !Array.isArray(listEntries)) {
    ...
    const validEntries = listEntries.every((entry: any) => 
      typeof entry === 'object' && 
    ...
    if (!validEntries) {
      failures += 1;
      const invalid = listEntries.find((entry: any) => 
        typeof entry !== 'object' || 
        typeof entry.name !== 'string' || 
        (entry.type !== 'file' && entry.type !== 'directory')
      );
  ...
  if (
    ...
    || (!memory_readJson.error.message.includes(memoryErrorPath) && !memory_readJson.error.message.includes(memoryErrorFile))
  ) {
    failures += 1;
    logLine('FAIL\ncase: memory read error\nexpected: path in error message\n\n', process.stderr);
  }
  ...
  if (!memory_writeJson || !memory_writeJson.error || memory_writeJson.error.code !== 'EXEC_ERROR') {
    failures += 1;
    logLine('FAIL\ncase: memory write error\nexpected: EXEC_ERROR\n\n', process.stderr);
  }
  if (
    ...
    || (!memory_writeJson.error.message.includes(memoryErrorPath) && !memory_writeJson.error.message.includes(memoryErrorFile))
  ) {
    failures += 1;
    logLine('FAIL\ncase: memory write error\nexpected: path in error message\n\n', process.stderr);
  }
  ...
  if (!missingArgsJson || !missingArgsJson.error || missingArgsJson.error.code !== 'MISSING_ARGUMENT') {
    failures += 1;
    logLine('FAIL\ncase: missing args\nexpected: VALIDATION_ERROR\n\n', process.stderr);
  }
  ...
  const validationCases = [
    ...
    },
    {
      ...
      payload: { mode: 'tool_call', tool_call: { tool_name: 'read_file', args: { path: 123 } } },
  ...
  if (!routerRecallJson || routerRecallJson.ok !== true || routerRecallText !== 'integration line') {
    failures += 1;
  ...
  const stdinErrorResult = spawnSync(process.execPath, ['-r', tsNodeRegister, '-e', stdinErrorScript], {
    cwd: spikeDir,
    encoding: 'utf8',
  });
  ...
  if (!unknownToolJson || !unknownToolJson.error || unknownToolJson.error.code !== 'UNKNOWN_TOOL') {
    failures += 1;
    logLine('FAIL\ncase: unknown tool\nexpected: VALIDATION_ERROR\n\n', process.stderr);
  }
  ...
  const memoryPathResult = runExecutorWithArgs(['--memory-path', '../bad.json'], memoryPathPayload);
  const memoryPathJson = parseOutput(memoryPathResult.stdout);
  if (!memoryPathJson || memoryPathJson.ok !== false) {
    failures += 1;
    logLine('FAIL\ncase: memory path\nexpected: ok false\n\n', process.stderr);
  }
  if (!memoryPathJson || !memoryPathJson.error || memoryPathJson.error.code !== 'VALIDATION_ERROR') {
    failures += 1;
    logLine('FAIL\ncase: memory path\nexpected: VALIDATION_ERROR\n\n', process.stderr);
  }
  ...
  try {
    ...
    fs.writeFileSync(corruptTasksPath, '{"id":1,"text":"valid 1","done":false}\nTHIS IS NOT JSON\n{"id":2,"text":"valid 2","done":false}\n', 'utf8');
  ...
  if (!offsetJson || offsetJson.ok !== true || offsetJson.result.content !== '' || offsetJson.result.eof !== true) {
    failures += 1;
    logLine('FAIL\ncase: read_file huge offset\nexpected: empty content eof true\ngot: ' + JSON.stringify(offsetJson) + '\n\n', process.stderr);
  }
  // Test node_modules block
  const nodeModulesPayload = { mode: 'tool_call', tool_call: { tool_name: 'run_cmd', args: { command: 'ls node_modules' } } };
